...
# ----------------------------------------------------------------------
# 🚀 6. 主程序运行与 PushPlus 终极防爆推送 (含加密暗盘情绪)
# ----------------------------------------------------------------------
def run_stock_picker():
    ...
    def build_cards(item_list, color_code):
        ...
        for item in item_list:
            ...
            <div style="border-left:4px solid {color_code}; background:#f9f9f9; padding:10px; margin-bottom:10px; border-radius:4px;">
                ...
                <div style="margin-top:4px; font-size:13px;"><b>现价:</b> {item['最新价']} | <b>涨跌:</b> {item['当日涨跌']} | <b>PE:</b> {item['PE']}</div>
    ...
    <div style="font-family: 'Helvetica Neue', Arial, sans-serif; color: #2c3e50;">
        ...
        <h4 style="color:#5cb85c; margin-bottom:5px;">🟢 【大师级买入标的 & 盈亏比/建仓参考】</h4>
    ...
    if PUSHPLUS_TOKEN and PUSHPLUS_TOKEN != "YOUR_PUSHPLUS_TOKEN":
        try:
            res = requests.post("http://www.pushplus.plus/send", json={
                ...
                "title": f"🌹 量化报告送达 (综合风险指数:{danger_count})",
                "content": content,
                "template": "html"
            }, timeout=15)
